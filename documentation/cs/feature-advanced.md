...
Pokud má váš teploměr atribut `last seen` indikující čas posledního kontaktu, můžete jej specifikovat v hlavních atributech _VTherm_ pro výrazné snížení falešných aktivací Bezpečnostního režimu. Viz [konfigurace](base-attributes.md#volba-základních-atributů) a [řešení problémů](troubleshooting.md#proč-můj-versatile-thermostat-přejde-do-režimu-bezpečnosti-).
